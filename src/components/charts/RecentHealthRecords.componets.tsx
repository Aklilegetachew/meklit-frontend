...
import { useRecentHealthRecords } from "../../services/healthServices/health.query"

export default function RecentHealthRecordsTable() {
  const { data, isLoading, error } = useRecentHealthRecords()
  
  if (isLoading) return <p>Loading...</p>
  if (error) return <p>Error fetching recent records</p>
            ...
            {data?.map((record: any, index: number) => (
              <TableRow key={index}>
                ...
                <TableCell className="py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                  <Badge
                    size="sm"
                    color={record.type === "Incident" ? "error" : "success"}
                  >
                    {record.type}
                  </Badge>
                </TableCell>
