...
import { useLogsOverTime } from "../../services/logsService/log.query"
import { Ellipsis } from "lucide-react"

type MonthlyTargetProps = {
  target: number // e.g. 200 logs
  centerId?: string // optional filter
}

export default function MonthlyTargetDailylog({
  target,
  ...
  const { data, isLoading } = useLogsOverTime(filters)
      ...
      <div className="px-5 pt-5 bg-white shadow-default rounded-2xl pb-11 dark:bg-gray-900 sm:px-6 sm:pt-6">
        <div className="flex justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
              Monthly Daily Log Target
            </h3>
            <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
              Progress toward your logs target this month
            </p>
          </div>
        ...
        <p className="mx-auto mt-10 w-full max-w-[380px] text-center text-sm text-gray-500 sm:text-base">
          {isLoading
            ...
            : `You’ve logged ${actual} daily entries this month. Target is ${target}.`}
        </p>
      </div>
    </div>
  )
